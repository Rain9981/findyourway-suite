...
def run():
    ...
    user_input = st.text_area("Enter prompt or info:", key="growth_input")
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="growth".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="growth_pdf"):
        ...
        c.drawString(100, 750, f"Report: growth".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'growth'.replace('_', ' ').title()}**.")
