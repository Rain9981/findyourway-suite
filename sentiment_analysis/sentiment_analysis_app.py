...
def run():
    st.title("AI Tool: " + "sentiment_analysis".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="sentiment_analysis_input")
    ...
    if st.button("Export to PDF", key="sentiment_analysis_pdf"):
        ...
        c.drawString(100, 750, f"Report: sentiment_analysis".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'sentiment_analysis'.replace('_', ' ').title()}**.")
