...
def run():
    st.title("AI Tool: " + "forecasting".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="forecasting_input")
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="forecasting".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="forecasting_pdf"):
        ...
        c.drawString(100, 750, f"Report: forecasting".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'forecasting'.replace('_', ' ').title()}**.")
