...
def run():
    st.title("AI Tool: " + "subscription_plans".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="subscription_plans_input")
    ...
    if st.button("Export to PDF", key="subscription_plans_pdf"):
        ...
        c.drawString(100, 750, f"Report: subscription_plans".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'subscription_plans'.replace('_', ' ').title()}**.")
