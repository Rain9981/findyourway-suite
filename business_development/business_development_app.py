...
def run():
    st.title("AI Tool: " + "business_development".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="business_development_input")
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="business_development".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="business_development_pdf"):
        ...
        c.drawString(100, 750, f"Report: business_development".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'business_development'.replace('_', ' ').title()}**.")
