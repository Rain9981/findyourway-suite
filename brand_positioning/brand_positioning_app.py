...
def run():
    st.title("AI Tool: Brand Positioning")
    prompt_label = "What would you like help with regarding Brand Positioning?"
    st.sidebar.header("💡 Guide")
    st.sidebar.write("**This tab helps you:**")
    st.sidebar.write("- Define your unique brand voice and market fit.")
    st.sidebar.write("- Use GPT-4o to enhance decision-making.")
    st.sidebar.write("- Save to Sheets or export a PDF.")

    user_input = st.text_area(prompt_label, key="brand_positioning_input")
    ...
    if st.button("Run GPT Analysis", key="brand_positioning_run") and user_input:
        try:
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant for Brand Positioning."},
    ...
    if st.button("Export to PDF", key="brand_positioning_pdf"):
        ...
        c.drawString(100, 750, "Consulting Report - Brand Positioning")
