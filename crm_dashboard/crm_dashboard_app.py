...
def run():
    st.title("AI Tool: " + "crm_dashboard".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="crm_dashboard_input")
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="crm_dashboard".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="crm_dashboard_pdf"):
        ...
        c.drawString(100, 750, f"Report: crm_dashboard".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'crm_dashboard'.replace('_', ' ').title()}**.")
