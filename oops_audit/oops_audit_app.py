...
def run():
    st.title("AI Tool: " + "oops_audit".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="oops_audit_input")
    ...
    if st.button("Export to PDF", key="oops_audit_pdf"):
        ...
        c.drawString(100, 750, f"Report: oops_audit".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'oops_audit'.replace('_', ' ').title()}**.")
