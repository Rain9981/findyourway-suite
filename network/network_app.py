...
def run():
    st.title("network".title() + " Tool")
    st.sidebar.header("💡 Consulting Guide")
    st.sidebar.markdown("**What this tab does:** Analyzes your 'network' strategy with AI.")
    ...
    if st.button("Run GPT Analysis", key="network_run") and prompt:
        try:
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a consulting AI specializing in network."},
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), {"input": prompt}, sheet_tab="network")
    ...
    if st.button("Export to PDF", key="network_pdf"):
        ...
        c.drawString(100, 750, "GPT Analysis for network")
