...
def run():
    ...
    user_input = st.text_area("Enter prompt or info:", key="network_input")
    ...
    if st.button("Export to PDF", key="network_pdf"):
        ...
        c.drawString(100, 750, f"Report: network".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'network'.replace('_', ' ').title()}**.")
