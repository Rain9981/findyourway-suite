...
def run():
    st.title("AI Tool: " + "strategy_designer".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="strategy_designer_input")
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="strategy_designer".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="strategy_designer_pdf"):
        ...
        c.drawString(100, 750, f"Report: strategy_designer".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'strategy_designer'.replace('_', ' ').title()}**.")
