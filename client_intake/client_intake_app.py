...
def run():
    st.title("AI Tool: " + "client_intake".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="client_intake_input")

    if st.button("Run GPT Analysis", key="client_intake_run") and user_input:
        try:
    ...
    try:
        save_data(st.session_state.get("user_role", "guest"), locals(), sheet_tab="client_intake".replace("_", " ").title())
    ...
    if st.button("Export to PDF", key="client_intake_pdf"):
        ...
        c.drawString(100, 750, f"Report: client_intake".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'client_intake'.replace('_', ' ').title()}**.")
