...
def run():
    st.title("AI Tool: " + "kpi_tracker".replace("_", " ").title())
    st.markdown("### Use GPT-4o to enhance your strategy or operations.")
    user_input = st.text_area("Enter prompt or info:", key="kpi_tracker_input")
    ...
    if st.button("Export to PDF", key="kpi_tracker_pdf"):
        ...
        c.drawString(100, 750, f"Report: kpi_tracker".replace("_", " ").title())
        y = 735
    ...
    with st.sidebar:
        st.markdown("## 💡 Guide")
        st.write(f"This tab helps analyze or build insights for **{'kpi_tracker'.replace('_', ' ').title()}**.")
